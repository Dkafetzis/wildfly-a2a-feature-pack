...
By default each module provisions the server from a hardcoded layer list. Passing `-Dts.glow`
activates the `glow.profile` in the JSON-RPC and REST modules, which instead scans the Arquillian
deployments with WildFly Glow and provisions from the discovered layer set, asserting it via
`expected-discovery`. This is what verifies the `org.wildfly.rule.*` metadata in the layer specs:

[source,bash]
----
mvn clean install -Dts.glow
----

The gRPC module is not part of that profile: it additionally requires the gRPC feature-pack and its
`grpc` layer, which no A2A layer depends on and which Glow cannot infer from a deployment.
