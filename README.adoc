...
`target/wildfly` from the hardcoded layer list, and `target/wildfly-glow` from the layer set WildFly
...
the build. Both run under a plain `mvn clean install`; results are told apart by the `-glow` suffix
on the surefire reports.
